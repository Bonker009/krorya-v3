...
export default function RecipeCard({
  recipeImage = 'https://via.placeholder.com/200x300',
  recipeTitle = 'បាយឆាផ្តើមស្នេហ៍',
  cookingTime = '25 នាទី',
  cookingLevel = 'ងាយស្រួល',
  recipeId
}: RecipeCardProps) {
  return (
    <Link
      href={`/explore-recipe/${recipeId}`}
      className="recipe-card sha carousel-item m-0 flex h-[90px] w-full overflow-hidden rounded-lg  bg-white shadow-md"
      style={{ margin: 0 }}
    >
      <div className="relative h-[90px] w-[90px]">
        <Image
          src={`${process.env.AWS_BASE_URL}${recipeImage}`}
          alt="recipe-image"
      ...
      <div className="flex flex-grow items-center justify-between p-3">
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs">{cookingTime} នាទី</span>
          </div>
          <div className="w-[80px] rounded-[6px] bg-[#FFEBBB] text-center">
            <span className="text-xs text-[#AE7C00] ">{cookingLevel}</span>
          </div>
        </div>
        <div className="h-[17px] w-[19px] self-start">
          <svg
            width="19"
            height="17"
            viewBox="0 0 34 34"
